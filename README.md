...
Apollo Client와 Fetch API를 함께 사용하여 각 통신 방식의 장점을 극대화했습니다.
</details>
...
❗ **Solved**  
pnpm workspace와 Turbo의 캐싱 전략을 최적화하여 공통 의존성은 루트에서 관리하고, 각 앱별 고유 의존성만 분리했습니다.  
개발 환경 안정성을 크게 향상시켰습니다.
</details>
...
❗ **Solved**  
사용자 피드백을 반영해 **핵심 경험에서는 일관된 로딩이 더 중요**하다고 판단, **Blocking Loading**으로 전환했습니다.  
기술적 완성도보다 **사용자 만족도를 우선**한 의사결정이었습니다.
</details>
...
❗ **Solved**  
일부 이미지를 **Cloudinary CDN**으로 마이그레이션하여 글로벌 캐싱과 자동 최적화를 적용했습니다.  
...
❗ **Solved**  
영어교육 전문가의 지식을 바탕으로 대화 데이터를 분석하고 정교하게 파인튜닝했습니다.  
실제 인간 선생님의 대화 스타일과 피드백 방식을 재현하여 자연스러운 교육적 상호작용을 구현했습니다.
</details>
...
❗ **Solved**  
**SERP API → 트렌드 분석 → 기사 생성 → 직역/의역 비교**의 다단계 프로세스를 구축했습니다.  
각 단계별로 서로 다른 모델과 프롬프팅 전략을 적용하여 **문화적 맥락이 살아있는 학습 콘텐츠**를 자동 생성했습니다.
</details>
...
❗ **Solved**  
**SendGrid Web API**로 전환하여 클라우드 기반 안정성을 확보했습니다.  
배치 처리와 에러 핸들링을 강화하여 안정적인 메일 전송 시스템을 구축했습니다.
</details>
...
❗ **Solved**  
**플랫폼별 어댑터 패턴**을 적용해 차이점을 추상화하고, **공통 인증 플로우**를 구축했습니다.  
각 플랫폼의 특성을 살리면서도 **일관된 사용자 경험**을 제공했습니다.
</details>
